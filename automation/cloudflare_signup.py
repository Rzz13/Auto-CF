...
def custom_email_request(email, path):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            # If path is fetching detail for a message ID (e.g. /messages/{id})
            if "messages/" in path or path.startswith("/messages/"):
                # Extract message ID from path, e.g. "/messages/custom_0"
                msg_id = path.split("/")[-1]
...
def create_token_via_session(page, default_account_id=""):
    ...
    try:
        ...
        if "dash.cloudflare.com" not in page.url or "email-verification" in page.url:
            log_step("Browser di halaman verifikasi/luar — mengarahkan ke dashboard utama...")
            try:
                page.goto("https://dash.cloudflare.com/", wait_until="domcontentloaded", timeout=25000)
                time.sleep(5)
            except Exception as _ge_redir:
                log_step(f"Warning redirect dashboard: {_ge_redir}")

        acc_id = ""
        # 1. Ambil Account ID via page.request.fetch (bypasses browser WAF/CORS restrictions)
        try:
            api_resp = page.request.fetch(
                "https://dash.cloudflare.com/api/v4/accounts?per_page=50",
                method="GET",
            ...
            log_step(f"Gagal mengambil Account ID via page.request: {e}")
        ...
        try:
            ...
            if api_resp.status in (200, 201):
                ...
                log_step(f"page.request /tokens HTTP error {api_resp.status}: {api_resp.text()[:200]}")
        except Exception as e:
            log_step(f"page.request token creation exception: {e}")
    except Exception as e:
        log_step(f"create_token_via_session exception: {e}")
    return None
    ...
    try:
        ...
        with opener.open(req, timeout=timeout) as response:
            if response.status in (200, 301, 302, 403, 404):
                print(f"Proxy alive: {proxy_url} (status {response.status})")
                return True
    except urllib.error.HTTPError:
        # An HTTP Error means we reached the target website, so proxy is alive
        return True
    except Exception:
        pass
    return False
...
def main():
    ...
    if proxy_dict:
        ...
        if not check_proxy_alive(proxy_dict):
            die("Proxy dead/InvalidIP - aborting to trigger new proxy retry", 3)
    ...
    with browser_ctx as browser:
        ...
        try:
            page.goto("https://dash.cloudflare.com/sign-up", wait_until="domcontentloaded", timeout=45000)
        except Exception as e:
            # Catat errornya untuk visibilitas, lalu gunakan 'commit' yang lebih toleran
            log_step(f"Warning: domcontentloaded gagal ({e}), mencoba fallback 'commit'...")
            try:
                page.goto("https://dash.cloudflare.com/sign-up", wait_until="commit", timeout=30000)
            except Exception as fatal_e:
                die(f"Gagal memuat halaman, koneksi proxy kemungkinan mati: {fatal_e}")
        ...
        if ammail_ok and not email_already_registered:
            ...
            if verify_link:
                ...
                try:
                    ...
                    # Selesaikan Turnstile menggunakan clearance handler teruji bawaan kita
                    ...
                    for _wait_url in range(10):
                        ...
                        log_step("Timeout / Belum redirect — Memaksa navigasi langsung ke dashboard...")
                        try:
                            page.goto("https://dash.cloudflare.com/", wait_until="commit", timeout=15000)
                        except Exception:
                            pass
                    time.sleep(2) # Jeda kecil stabilisasi setelah redirect
                    log_step(f"Navigasi verify link selesai")
                except Exception as e:
        ...
        # ── Step 7: AUTO-FREECE STYLE SESSION BYPASS (EKSEKUSI PENUH & INSTAN) ──
        # Tepat setelah email diverifikasi, kita panggil bypass API token menggunakan session cookies yang aktif di browser.
        ...
        if not email_already_registered:
            try:
                ...
                session_res = create_token_via_session(page, account_id)
                if session_res and session_res[0]:
                    session_token, session_acc_id = session_res
                    log_step("Berhasil mendapatkan API Token langsung via session bypass!")
                    success(session_token, session_acc_id or account_id, args.email)
                    sys.exit(0)
                else:
                    log_step("Bypass session gagal pada percobaan pertama, mencoba ulang dalam 5 detik...")
                    time.sleep(5)
                    session_res = create_token_via_session(page, account_id)
                    if session_res and session_res[0]:
                        session_token, session_acc_id = session_res
                        log_step("Berhasil mendapatkan API Token langsung via session bypass (retry)!")
                        success(session_token, session_acc_id or account_id, args.email)
                        sys.exit(0)
            except Exception as e:
                log_step(f"Gagal generate token via session bypass awal: {e}")

        # ── Step 8: Fallback Dashboard Login (Jika bypass session gagal) ────────
        log_step("Menunggu proses verifikasi dan redirect dashboard Cloudflare selesai (maks 15s)...")
        _early_account_id = ""
        for _wait_redir in range(15):
        ...
        # Coba ambil Account ID secara langsung via API menggunakan session cookie yang ada
        if not _early_account_id:
            try:
                log_step("Mencoba ambil Account ID langsung via API session...")
                api_resp = page.request.fetch(
                    "https://dash.cloudflare.com/api/v4/accounts?per_page=1",
                    method="GET",
                    headers={"Accept": "application/json"}
                )
                if api_resp.status == 200:
        ...
        try:
            ...
            session_res = create_token_via_session(page, account_id)
            if session_res:
                workers_ai_token, session_acc_id = session_res
                if session_acc_id:
                    account_id = session_acc_id
                log_step(f"Token via session fetch: {workers_ai_token[:12]}... Account ID: {account_id[:8]}...")
        except Exception as e:
        ...
        try:
            ...
            if not continue_clicked:
                ...
                try:
                    session_res = create_token_via_session(page, account_id)
                    if session_res:
                        api_token, session_acc_id = session_res
                        output_result({"status": "ok", "email": args.email, "api_key": api_token, "account_id": session_acc_id or account_id})
                        sys.exit(0)
                except Exception as e:
